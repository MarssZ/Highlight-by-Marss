...
```
3. **✨ 智能清理**：自动清除平台特定标记（如 Gemini 的引用编号），确保复制内容干净可用，代码块格式完整保留

**完整对话导出**：
1. 点击浏览器工具栏的扩展图标
2. 一键导出整个对话为结构化Markdown：
```markdown
...
你好！有什么可以帮助你的吗？
```
3. **✨ 保留完整格式**：使用 Turndown 库，完美保留代码块、列表、加粗等Markdown格式
...
- **🆕 完整对话导出**：一键导出整个对话为结构化Markdown，支持Gemini平台（其他平台开发中）
- **🆕 格式完美保留**：集成Turndown库，完整保留代码块、列表、加粗等Markdown格式
