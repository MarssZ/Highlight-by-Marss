...
- [ ] 8. 实现存储数据结构
  - 需求：REQ-1.4 页面刷新保持高亮
  - 文件：`src/storage.js`
  - 验证：高亮文本后，Chrome扩展存储中看到保存的高亮数据

- [ ] 9. 实现页面加载时恢复高亮
  - 需求：REQ-1.4 保持高亮状态
  - 文件：`src/restore.js`
  - 验证：高亮文本→刷新页面→高亮效果依然存在

- [ ] 10. 优化存储键生成逻辑
  - 需求：数据可靠性
  - 文件：`src/storage.js`（优化）
  - 验证：在不同Gemini对话中高亮，数据不会互相干扰

### 稳定性增强功能

- [ ] 11. 添加动态内容监听优化
  - 需求：处理动态加载的AI回复
  - 文件：`src/content.js`（MutationObserver优化）
  - 验证：在长对话中新生成的AI回复也能正常高亮和复制
